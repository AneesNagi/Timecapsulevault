...
export const WalletManager = () => {
  ...
  const getMockTransactions = (): TransactionPreview[] => {
    return [
      {
        hash: '0x1234...5678',
        type: 'receive',
  ...
  useEffect(() => {
    console.log('🔄 WalletManager: Network change detected!', selectedNetwork);
    console.log('🔄 WalletManager: Current wallets count:', wallets.length);
    
    if (wallets.length > 0 && selectedNetwork) {
      console.log('WalletManager: Network changed, refreshing balances for', selectedNetwork.name);
      console.log('WalletManager: Current wallets before refresh:', wallets);
      setTimeout(() => {
        refreshBalances();
      }, 1000); // Small delay to ensure network change is complete
    } else {
      console.log('WalletManager: Skipping balance refresh - no wallets or no selected network');
    }
  ...
  const refreshBalances = useCallback(async () => {
    console.log('🚀 WALLETMANAGER REFRESH BALANCES STARTED 🚀');
    ...
    try {
      ...
      const updatedWallets = await Promise.all(
        currentWallets.map(async (wallet: WalletData) => {
          try {
            // Use selectedNetwork instead of wallet.network for balance fetching
            const network = selectedNetwork;
            if (!network) {
              console.warn(`Selected network not found`);
              return wallet;
            }
            
            // Try multiple RPC endpoints for better reliability
            console.log(`WalletManager: Fetching balance for ${wallet.address} on ${network.name} (${network.id})`);
            let balance = null;
      ...
      console.log('WalletManager: Updated wallets before saving:', updatedWallets.map(w => ({ address: w.address, balance: w.balance, network: w.network })));
      setWallets(updatedWallets);
      localStorage.setItem('wallets', JSON.stringify(updatedWallets));
            ...
            <VStack spacing={6} align="stretch">
              ...
              {loading ? (
                      ...
                      >
                        {[1, 2, 3, 4, 5, 6].map((i) => (
                          <EnhancedSkeletonCard 
                            ...
                            variant="wallet" 
                            index={i - 1} 
                            height={mobileOpt.isMobile ? "180px" : "200px"}
                          />
                        ))}
                      </SimpleGrid>
              ) : wallets.length > 0 ? (
                                        ...
                                        <HStack key={txIndex} justify="space-between" p={2} bg="rgba(127, 90, 240, 0.1)" borderRadius="md" border="1px solid" borderColor={borderColor}>
                                          <HStack spacing={2}>
                                            ...
                                            <Text fontSize="xs" color={textColor}>
                                              {tx.amount} {selectedNetwork.currency}
                              </Text>
                                          </HStack>
                                          <HStack spacing={1}>
                                            <Icon 
                            ...
                            <VStack spacing={2} w="full">
                              <Button 
                                ...
                                onClick={() => {
                                  ...
                                  toast({
                                    title: 'Address Copied! 📋',
                                    description: `Get testnet ${selectedNetwork.currency} from a faucet to start using your wallet`,
                                    status: 'info',
                                    duration: 5000,
                                  });
                ...
                window.open(faucetUrl, '_blank');
                                ...
                                transition="all 0.2s"
                        >
                                Get Test {selectedNetwork.currency}
                              </Button>
                          <Button 
                            width="full" 
                            onClick={() => viewWalletDetails(wallet)} 
                  ...
                  <CardBody>
                    ...
                    />
                      ...
                      <Box width="full">
                        ...
                        <Center>
                          <Box p={3} bg="rgba(35, 37, 38, 0.9)" borderRadius="lg">
